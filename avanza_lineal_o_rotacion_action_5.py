...
from tf_transformations import euler_from_quaternion  # Importar euler_from_quaternion desde tf.transformations
...
class RobotController(Node): 
    def __init__(self): 
        ...
        self.srv = ActionServer (self, LlamadaRobot, 'move_robot', self.move_robot_callback, callback_group=self.reentrant_group) 
    ...
    def odom_callback(self, msg): 
        ...
        # _, _, yaw = euler_from_quaternion(
            # [msg.pose.pose.orientation.x, msg.pose.pose.orientation.y,
             # msg.pose.pose.orientation.z, msg.pose.pose.orientation.w]) #HE COLOCADO ESTO        
        self.odom_orientation_z = msg.pose.pose.orientation.z  # Actualiza la orientación del robot 
        # self.odom_orientation_z = yaw 
    ...
    def rotate_to_target(self, target_angle): 
        ...
        self.feedback.angle_to_target = error_angle     
    ...
    def move_to_target(self, linear=True, value=1.0, angle_in_degrees=True, handle=None): 
        ...
        handle.publish_feedback(self.feedback)
        self.stop()  # Asegura que el robot se detenga al final HE CAMBIADO ESTO
        
    def move_robot_callback(self, handle): 
        ...
        linear = handle.request.linear 
        print (linear)
        value = handle.request.value 
        ...
        while self.started: # continuará ejecutándose mientras el atributo "started" sea True. Dentro del bucle, se llama a la función 
            self.move_to_target(linear, value, True, handle) 
        ...
        handle.succeed() # HE COLOCADO ESTO
        return response 
